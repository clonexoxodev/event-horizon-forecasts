...
-- Trigger: Notify user when forecast is confirmed
CREATE OR REPLACE FUNCTION notify_forecast_confirmed()
RETURNS TRIGGER AS $$
BEGIN
  PERFORM create_notification(
    ...
    'forecast_confirmed',
    'Forecast Confirmed',
    format('Your %s forecast of ₦%s on "%s" is active.',
      NEW.side,
      (NEW.amount / 100.0)::TEXT,
      (SELECT question FROM markets WHERE id = NEW.market_id)
    ),
    jsonb_build_object(
...
-- Note: This trigger will be created after positions table exists
-- DROP TRIGGER IF EXISTS trigger_notify_forecast_confirmed ON positions;
-- CREATE TRIGGER trigger_notify_forecast_confirmed
--   AFTER INSERT ON positions
--   FOR EACH ROW
--   EXECUTE FUNCTION notify_forecast_confirmed();
